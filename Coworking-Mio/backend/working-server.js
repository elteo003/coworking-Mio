...
console.log('🚀 Avvio server professionale...');
...
async function handleRequest(req, res) {
    ...
    try {
        ...
        if (path === '/api/ping' && method === 'GET') {
            sendJSON(res, {
                message: 'Server professionale funzionante!',
        ...
        if (path === '/api/dashboard/stats' && method === 'GET') {
            ...
            // Query per statistiche con gestione errori
            try {
        ...
        // Route: /api/analytics/event (per gestire le richieste del frontend)
        if (path === '/api/analytics/event' && method === 'POST') {
            const body = await readBody(req);
            // Log silenzioso per non saturare l'output
...
// Avvio server
server.listen(PORT, () => {
    console.log(`✅ Server professionale avviato sulla porta ${PORT}`);
