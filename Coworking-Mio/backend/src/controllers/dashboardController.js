...
const getDashboardStats = async (req, res) => {
    try {
        ...
        const prenotazioniQuery = `
            ...
            ${sedeFilter.replace('WHERE', 'AND')}
        `;

        // Query per fatturato giornaliero - Pagamenti ricevuti oggi
        const fatturatoQuery = `
            SELECT COALESCE(SUM(pa.importo), 0) as fatturato_giorno
            FROM Pagamento pa
            JOIN Prenotazione p ON pa.id_prenotazione = p.id_prenotazione
            JOIN Spazio s ON p.id_spazio = s.id_spazio
            WHERE DATE(pa.data_pagamento) = CURRENT_DATE
            AND pa.stato = 'pagato'
            ${sedeFilter.replace('WHERE', 'AND')}
        `;
        ...
        let prenotazioniResult, fatturatoResult, occupazioneResult;

        try {
            [prenotazioniResult, fatturatoResult, occupazioneResult] = await Promise.all([
...
const getDashboardCharts = async (req, res) => {
    try {
        ...
        // Query per occupazione per spazio - Calcolo percentuale basato su giorni con prenotazioni
        const occupazioneQuery = `
            SELECT 
                s.nome,
                ROUND(
                    (COUNT(DISTINCT DATE(p.data_inizio)) * 100.0 / 
                    GREATEST(${periodo}, 1)), 2
                ) as occupazione
            FROM Spazio s
            LEFT JOIN Prenotazione p ON s.id_spazio = p.id_spazio 
                AND p.data_inizio >= CURRENT_DATE - INTERVAL '${periodo} days'
                AND p.data_fine <= CURRENT_DATE + INTERVAL '1 day'
                AND p.stato = 'confermata'
            ${sedeFilter}
