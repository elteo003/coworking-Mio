...
exports.getPrenotazioni = async (req, res) => {
  const { utente, gestore, tipo, sede, data_inizio, data_fine, stato } = req.query;
  const userId = req.user?.id_utente;

  console.log('📋 Prenotazioni - Richiesta:', { utente, gestore, tipo, sede, data_inizio, data_fine, stato, userId });
  ...
  try {
    ...
    if (utente) {
      ...
      query = `
        ...
        WHERE p.id_utente = $${paramIndex}
      `;
      params.push(utente);
      paramIndex++;
    } else if (gestore) {
      // Query per gestore specifico
      query = `
        SELECT p.*, s.nome AS nome_spazio, se.nome AS nome_sede, se.indirizzo AS indirizzo_sede,
               u.nome AS nome_utente, u.cognome AS cognome_utente, u.email AS email_utente
        FROM Prenotazione p
        ...
        JOIN Utente u ON p.id_utente = u.id_utente
        WHERE se.id_sede IN (
          SELECT id_sede FROM Sede WHERE id_gestore = $${paramIndex}
        )
      `;
      params.push(gestore);
      paramIndex++;
    } else if (tipo === 'responsabile') {
      // Query per responsabile (nuovo formato)
      query = `
        SELECT p.*, s.nome AS nome_spazio, se.nome AS nome_sede, se.indirizzo AS indirizzo_sede,
               u.nome AS nome_utente, u.cognome AS cognome_utente, u.email AS email_utente
        FROM Prenotazione p
      ...
      if (sede && sede !== '' && sede !== 'undefined') {
        query += ` AND s.id_sede = $${paramIndex}`;
        params.push(sede);
        paramIndex++;
      }

      // Filtro per data inizio se specificata
      if (data_inizio && data_inizio !== '') {
        query += ` AND DATE(p.data_inizio) >= $${paramIndex}`;
        params.push(data_inizio);
        paramIndex++;
      }

      // Filtro per data fine se specificata
      if (data_fine && data_fine !== '') {
        query += ` AND DATE(p.data_fine) <= $${paramIndex}`;
      ...
      return res.status(400).json({ error: 'Fornire utente, gestore o tipo=responsabile' });
    }

    // Aggiungi ordinamento
    query += ` ORDER BY p.data_inizio DESC`;

    console.log('📋 Prenotazioni - Query:', query);
    console.log('📋 Prenotazioni - Parametri:', params);
