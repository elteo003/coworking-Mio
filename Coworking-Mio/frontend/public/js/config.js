...
function updateNavbarUniversal() {
    ...
    if (userStr) {
        try {
            ...
            if (dashboardLink) {
                dashboardLink.textContent = `Dashboard ${user.nome}`;
                // Il tasto Dashboard personale porta sempre alla dashboard personale
                dashboardLink.href = 'dashboard.html';
            }

            console.log('✅ Navbar aggiornata con info utente centralizzata');

            // Aggiungi Dashboard Gestori per gestori e amministratori (separata dalla dashboard personale)
            if (user.ruolo === 'gestore' || user.ruolo === 'amministratore') {
                const dashboardGestoriItem = `
                    <li class="nav-item">
                        <a class="nav-link" href="dashboard-responsabili.html">
                            <i class="fas fa-users-cog me-1"></i>Dashboard Gestori
                        </a>
                    </li>
                `;
                // Inserisci dopo il tasto Dashboard personale
                const dashboardLink = document.getElementById('dashboardLink');
                if (dashboardLink) {
                    dashboardLink.parentNode.insertAdjacentHTML('afterend', dashboardGestoriItem);
                }
            }
...
function showNavbarForUnauthenticatedUser(config) {
    ...
    // Rimuovi eventuali tasti Dashboard Gestori rimasti
    const dashboardGestoriLinks = document.querySelectorAll('a[href="dashboard-responsabili.html"]');
    dashboardGestoriLinks.forEach(link => {
        const navItem = link.closest('.nav-item');
        if (navItem && navItem !== dashboardLink?.parentNode) {
            navItem.remove();
        }
    });
...
// Carica Token Expiry Manager
function loadTokenExpiryManager() {
    // Carica CSS
    if (!document.querySelector('link[href*="token-expiry-modal.css"]')) {
        const link = document.createElement('link');
        link.rel = 'stylesheet';
        link.href = 'css/token-expiry-modal.css';
        document.head.appendChild(link);
        console.log('✅ CSS Token Expiry Manager caricato');
    }

    // Carica JS
    if (!document.querySelector('script[src*="token-expiry-manager.js"]')) {
        const script = document.createElement('script');
        script.src = 'js/token-expiry-manager.js';
        script.onload = () => {
            console.log('✅ Token Expiry Manager JS caricato');
        };
...
document.addEventListener('DOMContentLoaded', loadTokenExpiryManager);
