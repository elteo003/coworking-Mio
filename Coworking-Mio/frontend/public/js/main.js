...
// Login
window.handleLogin = async function (event, email, password) {
  console.log('🚀 handleLogin chiamato con:', { event: !!event, email, password: password ? '***' : 'undefined' });
  ...
  // Usa async/await per migliore gestione errori
  try {
    const response = await fetch(`${window.CONFIG.API_BASE}/login`, {
      method: 'POST',
      headers: {
    ...
    console.log('🔍 Risposta login ricevuta:', responseData);
    console.log('🔍 Tipo risposta:', typeof responseData);
    console.log('🔍 Chiavi disponibili:', Object.keys(responseData));
    console.log('🔍 Token presente:', 'token' in responseData);
    console.log('🔍 Valore token:', responseData.token);
    console.log('🔍 Ruolo utente:', responseData.ruolo);
    console.log('🔍 Ruolo utente (tipo):', typeof responseData.ruolo);
    console.log('🔍 Ruolo utente (valore esatto):', JSON.stringify(responseData.ruolo));
    ...
    // Debug: controlla se c'è un redirect salvato
    const redirectAfterLogin = localStorage.getItem('redirectAfterLogin');
    console.log('🔍 Redirect salvato:', redirectAfterLogin);

    // CONTROLLO PREVENTIVO: Se c'è un redirect verso dashboard-responsabili ma l'utente è normale, rimuovilo
    if (redirectAfterLogin && redirectAfterLogin.includes('dashboard-responsabili.html')) {
      const userRole = responseData?.ruolo;
      const normalizedRole = userRole?.toLowerCase();
      console.log('🚨 CONTROLLO PREVENTIVO: Redirect verso dashboard-responsabili per utente con ruolo:', normalizedRole);

      if (normalizedRole !== 'gestore' && normalizedRole !== 'amministratore') {
        console.log('🔧 PULIZIA: Rimuovo redirect non autorizzato verso dashboard-responsabili');
    ...
    if (redirectAfterLogin) {
      ...
      // CONTROLLO SPECIALE: Se il redirect è verso dashboard-responsabili ma l'utente è normale, correggi
      if (redirectAfterLogin.includes('dashboard-responsabili.html')) {
        const userRole = responseData?.ruolo;
        const normalizedRole = userRole?.toLowerCase();
        console.log('🚨 ATTENZIONE: Redirect verso dashboard-responsabili per utente con ruolo:', normalizedRole);

        if (normalizedRole !== 'gestore' && normalizedRole !== 'amministratore') {
          console.log('🔧 CORREZIONE: Utente normale reindirizzato a dashboard utente invece di dashboard responsabili');
          localStorage.removeItem('redirectAfterLogin');
          setTimeout(() => {
      ...
      if (redirectAfterLogin.includes('selezione-slot.html')) {
        ...
        if (pendingPrenotazione) {
          try {
            ...
            // Fallback: vai alla dashboard appropriata basata sul ruolo (case-insensitive)
            const userRole = responseData?.ruolo;
            const normalizedRole = userRole?.toLowerCase();
            console.log('🔍 Fallback - Ruolo normalizzato:', normalizedRole);
          ...
          // ✅ REDIRECT BASATO SUL RUOLO (redirect fallback)
          const userRole = responseData?.ruolo;
          console.log('🔍 handleLogin - Ruolo utente (redirect fallback):', userRole);
          console.log('🔍 handleLogin - responseData completo:', responseData);

          // Distingui tra gestori e utenti normali (case-insensitive)
          const normalizedRole = userRole?.toLowerCase();
          console.log('🔍 Ruolo normalizzato:', normalizedRole);

          if (normalizedRole === 'gestore' || normalizedRole === 'amministratore') {
            console.log('🎯 handleLogin - Redirect fallback, gestore va a dashboard-responsabili.html');
            setTimeout(() => {
              window.location.href = 'dashboard-responsabili.html';
            }, 1000);
          } else {
            console.log('🎯 handleLogin - Redirect fallback, utente normale va a dashboard.html (ruolo:', userRole, ', normalizzato:', normalizedRole, ')');
            setTimeout(() => {
              window.location.href = 'dashboard.html';
            }, 1000);
          }
    ...
    if (pendingPrenotazione) {
      ...
      if (isFromHome) {
        ...
        // ✅ REDIRECT BASATO SUL RUOLO (login dalla home)
        const userRole = responseData?.ruolo;
        console.log('🔍 handleLogin - Ruolo utente (login dalla home):', userRole);
        console.log('🔍 handleLogin - responseData completo (home):', responseData);

        // Distingui tra gestori e utenti normali (case-insensitive)
        const normalizedRole = userRole?.toLowerCase();
        console.log('🔍 Ruolo normalizzato (home):', normalizedRole);

        if (normalizedRole === 'gestore' || normalizedRole === 'amministratore') {
          console.log('🎯 handleLogin - Login dalla home, gestore va a dashboard-responsabili.html');
          showAlert('Login effettuato! Hai una prenotazione in attesa che puoi completare dalla dashboard.', 'info');
          setTimeout(() => {
            window.location.href = 'dashboard-responsabili.html';
          }, 1000);
        } else {
          console.log('🎯 handleLogin - Login dalla home, utente normale va a dashboard.html (ruolo:', userRole, ', normalizzato:', normalizedRole, ')');
          showAlert('Login effettuato! Hai una prenotazione in attesa che puoi completare dalla dashboard.', 'info');
          setTimeout(() => {
            window.location.href = 'dashboard.html';
          }, 1000);
      ...
      // ✅ REDIRECT BASATO SUL RUOLO (se non stanno prenotando)
      const userRole = responseData?.ruolo;
      console.log('🔍 handleLogin - Ruolo utente:', userRole);
      console.log('🔍 handleLogin - responseData completo (principale):', responseData);

      // Distingui tra gestori e utenti normali (case-insensitive)
      const normalizedRole = userRole?.toLowerCase();
      console.log('🔍 Ruolo normalizzato (principale):', normalizedRole);

      if (normalizedRole === 'gestore' || normalizedRole === 'amministratore') {
        console.log('🎯 handleLogin - Login come azione principale, gestore va a dashboard-responsabili.html');
        setTimeout(() => {
          window.location.href = 'dashboard-responsabili.html';
        }, 1000);
      } else {
        console.log('🎯 handleLogin - Login come azione principale, utente normale va a dashboard.html (ruolo:', userRole, ', normalizzato:', normalizedRole, ')');
        setTimeout(() => {
          window.location.href = 'dashboard.html';
        }, 1000);
      }
...
// Registrazione
window.handleRegistration = async function (event, nome, cognome, email, password, telefono, ruolo) {
  if (event && event.preventDefault) {
    event.preventDefault();
  }
  ...
  try {
    const response = await fetch(`${window.CONFIG.API_BASE}/register`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    console.log('🔍 Risposta registrazione ricevuta:', responseData);
...
function setupPasswordToggles() {
  // Non aggiungere toggle password se siamo su login.html (ha i suoi toggle)
  const currentPage = window.location.pathname.split('/').pop() || 'index.html';
  if (currentPage === 'login.html') {
    console.log('Skip password toggles - siamo su login.html che ha i suoi toggle');
    return;
  }
