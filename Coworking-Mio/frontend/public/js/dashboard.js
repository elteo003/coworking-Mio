...
function displayPrenotazioniUtente(prenotazioni) {
  ...
  prenotazioni.forEach(p => {
    ...
    // Controlla se la prenotazione è scaduta (sia per stato che per tempo)
    const isScaduta = p.stato === 'scaduta' || tempoRimanente === 'SCADUTO';
    ...
    if (isScaduta) {
      ...
      azioniHtml = `
        <div class="btn-group" role="group">
          ...
          <button class="btn btn-danger btn-sm" onclick="cancellaPrenotazione(${p.id_prenotazione})" title="Cancella prenotazione scaduta">
            ❌ Cancella
          </button>
        </div>
      `;
    ...
    html += `
      <tr class="${rowClass}">
        ...
        <td><span class="badge bg-${getStatusColor(isScaduta ? 'scaduta' : p.stato)}">${isScaduta ? 'SCADUTO' : p.stato}</span></td>
