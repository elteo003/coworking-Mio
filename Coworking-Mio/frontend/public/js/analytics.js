...
class AnalyticsSystem {
    ...
    interceptAPICalls() {
        // Evita di intercettare se già fatto
        if (window.fetch._analyticsIntercepted) {
            return;
        }
        ...
        window.fetch = async (...args) => {
            ...
            try {
                ...
                // Traccia solo se non è una richiesta di analytics
                if (!args[0].includes('/analytics/') && !args[0].includes('/ab-testing/')) {
        ...
        // Marca come intercettato per evitare duplicazioni
        window.fetch._analyticsIntercepted = true;
    }
...
// Funzioni di utilità per analytics
function getConversionCampaign() {
    const urlParams = new URLSearchParams(window.location.search);
    return urlParams.get('utm_campaign') || 'none';
}
